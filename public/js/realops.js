...
function fixTime(el) { // Fixes times to HH:MM format
...
function fixDate(el) { // Fixes dates to YYYY-MM-DD format
    let dateStr = document.getElementById(el).value;
    let currDate = new Date();
    let currYear = currDate.getUTCFullYear().toString();
    if(dateStr.match(/^(\d{4}|\d{2})-\d{1,2}-\d{1,2}$/)) { // Format YY-MM-DD, YYYY-M-D
        let dateArr = document.getElementById(el).value.split('-');
        if(dateArr[0].length == 2) {
            dateArr[0] = currYear.substr(0,2) + dateArr[0];
        }
        document.getElementById(el).value = dateArr[0] + '-' + dateArr[1].padStart(2,'0') + '-' + dateArr[2].padStart(2,'0');
        return true;
    }
    else if(dateStr.match(/^\d{1,2}\/\d{1,2}\/(\d{4}|\d{2})$/)) { // MM/DD/YYYY, MM/DD/YY, M/D/YY
        let dateArr = document.getElementById(el).value.split('/');
        if(dateArr[2].length == 2) {
            dateArr[2] = currYear.substr(0,2) + dateArr[2];
        }
        document.getElementById(el).value = dateArr[2] + '-' + dateArr[0].padStart(2,'0') + '-' + dateArr[1].padStart(2,'0');
        return true;
    }
    return false;
}
