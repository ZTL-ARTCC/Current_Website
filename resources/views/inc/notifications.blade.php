@if(
    Auth::user()->hasRole('ins') && App\Training\Ots::where('status', 0)->get()->count() > 0 || Auth::user()->hasRole('atm') && App\Training\Ots::where('status', 0)->get()->count() > 0 ||
    App\Training\Ots::where('status', 1)->where('ins_id', Auth::id())->get()->count() > 0 ||
    App\Training\TrainingTicket::where('created_at', '>=', Carbon\Carbon::now()->subHours(24))->where('controller_id', Auth::id())->first() != null ||
    count(App\Mship\Incident::where('status', 0)->get()) > 0 && Auth::user()->can('snrStaff') ||
    count(App\Mship\Feedback::where('status', 0)->get()) > 0 && Auth::user()->can('snrStaff') ||
    count(App\Mship\Visitor::where('status', 0)->get()) > 0 && Auth::user()->can('snrStaff')
    )
    <hr>
    <center><h4><i>Notifications</i></h4></center>
    @if(Auth::user()->hasRole('ins') && App\Training\Ots::where('status', 0)->get()->count() > 0 || Auth::user()->hasRole('atm') && App\Training\Ots::where('status', 0)->get()->count() > 0)
        <br>
