...
<div class="container">
								...
								<div class="btn-group">
                                    ...
                                    @if(Auth::user()->isAbleTo('files'))
										...
										@if(!is_null($f->permalink))
											<a onclick="linkToClipboard(this);" class="btn btn-secondary simple-tooltip" data-toggle="tooltip" title="/asset/{{ $f->permalink }}"><i class="fas fa-link"></i></a>
										@endif
										@if(!$loop->first)
											<a onclick="itemReorder({{ $f->id }},{{ $loop->index }},{{ $f->type }},'up');" class="btn btn-info simple-tooltip" data-toggle="tooltip" title="Up"><i class="fas fa-arrow-up"></i></a>
										@endif
	...
	function linkToClipboard(e) {
		var path = getSiteRoot() + e.getAttribute('title');
