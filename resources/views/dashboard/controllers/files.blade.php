...
<div class="container">
    ...
    <div class="tab-content">
                                ...
                                <tr>
                                    @if(Auth::user()->isAbleTo('files'))
                                        ...
                                        <td>
                                            ...
                                            @if(!$loop->first)
	    										<a onclick="itemReorder({{ $f->id }},{{ $loop->index }},{{ $f->type }},'up',{{ $f->row_separator }});" class="btn btn-info simple-tooltip" data-toggle="tooltip" title="Up"><i class="fas fa-arrow-up"></i></a>
		    								@endif
			    							@if(!$loop->last)
				    							<a onclick="itemReorder({{ $f->id }},{{ $loop->index }},{{ $f->type }},'down',{{ $f->row_separator }});" class="btn btn-info simple-tooltip" data-toggle="tooltip" title="Down"><i class="fas fa-arrow-down"></i></a>
					    					@endif
                                        </td>
                                    @else
        ...
        @endif
		<script>
		function itemReorder(id,pos,typ,act,sep=0) { // Handles custom re-ordering of items in file browser
			var dType = '';
			switch(typ) {
			...
			$.get('/dashboard/admin/files/disp-order?id=' + id + '&pos=' + pos + '&act=' + act + '&typ=' + typ + '&sep=' + sep, function(data) {
				if(data.length > 0) {
