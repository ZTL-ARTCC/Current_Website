                    ...
                    @if($vrc->count() > 0)
                        @foreach($vrc as $f)
                            <tr>
                                ...
                                <td>
								<div class="btn-group">
                                    <a href="{{ $f->path }}" download="<?php echo basename($f->path); ?>" target="_blank" class="btn btn-success simple-tooltip" data-toggle="tooltip" title="Download"><i class="fas fa-download"></i></a>
                                    @if(Auth::user()->isAbleTo('files'))
