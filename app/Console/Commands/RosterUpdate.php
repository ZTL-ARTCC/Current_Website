...
class RosterUpdate extends Command {
    ...
    public function handle() {
        $client = new Client();
        $res = $client->get(Config::get('vatusa.base').'/v2/facility/'.Config::get('vatusa.facility').'/roster/both?apikey='.Config::get('vatusa.api_key'));
        ...
        foreach ($roster->data as $r) {
            ...
            $user->email =  (Config::get('app.env') == 'local') ? $this->option('local_email') : $user->email = $r->email;
            $user->rating_id = $r->rating;
            if ($r->facility == Config::get('vatusa.facility')) {
                $user->visitor = 0;
                $user->visitor_from = null;
            } else {
                $user->visitor = 1;
                $user->visitor_from = $r->facility;
            }
            $user->status = 1;
        ...
        $users = User::where('status', 1)->where('api_exempt', 0)->get();
        foreach ($users as $u) {
            $delete = true;
            foreach ($roster->data as $r) {
                if ($u->id == $r->cid) {
